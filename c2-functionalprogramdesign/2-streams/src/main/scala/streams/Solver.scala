...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    ...
    } yield (newBlock, move :: history)).to(LazyList) 
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    for {
      (newBlock, moveList) <- neighbors if !(explored contains newBlock)
    } yield (newBlock, moveList)
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val initialPaths = LazyList((startBlock, List()))
    val initialExplored = Set(startBlock)
    from(initialPaths, initialExplored)
  }
  ...
  lazy val solution: List[Move] = {
    // As paths are ordered by length due to searching breadth-first, the
    // shortest path (or one of them) can be found as the first element 
