  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) =>
        ...
        if (decodedItems.exists(_.isEmpty)) None
        else Some(decodedItems.map(_.head))
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] = {
    Decoder.fromPartialFunction {
      case Json.Obj(fields) =>
        decoder.decode(fields.get(name).head).head
    }
  }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](personTuple => Person(personTuple._1, personTuple._2))
}
...
trait ContactsCodecs {
  ...
  /** The encoder for `Contacts` */
    /*
  implicit val contactsEncoder: Encoder[List[Person]] =
    ObjectEncoder.field[String]("name")
      .transform[Person](user => (user.name, user.age))
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val contactsJson = parseJson(""" [{ "name": "Alice", "age": 42 }, { "name": "Bob", "age": 66 }]""")
