...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map((lang: String) => (lang, occurrencesOfLang(lang, rdd))).sortBy((artTuple: Tuple2[String, Int]) => artTuple._2)(Ordering.Int.reverse)

  /* Compute an inverted index of the set of articles, mapping each language
   * to the Wikipedia pages in which it occurs.
   */
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd
    .flatMap((art: WikipediaArticle) => for {lang <- langs if art.mentionsLanguage(lang)} yield (lang, art))
    .groupByKey
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd
    .flatMap((art: WikipediaArticle) => for {lang <- langs if art.mentionsLanguage(lang)} yield (lang, 1))
    .reduceByKey(_ + _)
    .collect
    .sortBy(_._2)(Ordering.Int.reverse)
    .toList
