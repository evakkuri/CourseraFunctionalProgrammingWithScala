...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageByLifePeriodOwn(): Unit = {
    val (columns, initDf) = read("src/main/resources/timeusage/atussum.csv")
    //initDf.show(5)

    val testRow = row(List("foo", "1.0", "2.0"))
    //println(testRow)
    ...
    //val finalDf = timeUsageGrouped(summaryDf)
    //finalDf.show(5)
    //timeUsageGroupedSql(summaryDf).show(5)

    val summaryDataset = timeUsageSummaryTyped(summaryDf)
    summaryDataset.show(5)

    val groupedDataset: Dataset[TimeUsageRow] = timeUsageGroupedTyped(summaryDataset)
    groupedDataset.show(5)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workingActs = columnNames
      .filter((colName: String) =>
        colName.startsWith("t05") | colName.startsWith("t1805"))
      .map(col)

    val otherActsPrefixes = List(
      "t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18"
    )

    val otherActs = columnNames
      .filter((colName: String) =>
        otherActsPrefixes.contains(colName.substring(0, 3)))
      .filter((colName: String) =>
        !colName.startsWith("t1801") & !colName.startsWith("t1803") & !colName.startsWith("t1805"))
      .map(col)

    (primaryNeeds, workingActs, otherActs)
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column =
      when($"teage" >= 15 && $"teage" < 23, "young")
        .when($"teage" >= 23 && $"teage" < 56, "active")
        .otherwise("elder")
        .as("age")
