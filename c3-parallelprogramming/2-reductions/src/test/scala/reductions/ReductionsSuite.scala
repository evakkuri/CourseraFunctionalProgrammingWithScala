...
class ReductionsSuite {
  ...
  @Test def `parallel balance should work for string of length 2 and threshold 1`: Unit = {
    def check(input: String, expected: Boolean) =
  ...
  @Test def `parBalance should work for nested parentheses and threshold 1`: Unit = {
    def check(input: String, expected: Boolean) =
      assert(parBalance(input.toArray, 1) == expected,
        s"balance($input) should be $expected")
