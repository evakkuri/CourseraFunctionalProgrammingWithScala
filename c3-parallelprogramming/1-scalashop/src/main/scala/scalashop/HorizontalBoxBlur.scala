...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface {
  ...
  def blur(src: Img, dst: Img, from: Int, end: Int, radius: Int): Unit = {
    ...
    for (y <- from until end; x <- 0 until dst.width) {
      //println(s"x $x, y $y")
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    ...
    val bandWidth =
      if (numTasks >= src.height) 1
      else src.height / numTasks
    val startYs = 0 until src.height by bandWidth
    val bandEdges = startYs map ((startY: Int) =>
      (startY, math.min(startY + bandWidth, src.height)))
    //println(bandEdges)
